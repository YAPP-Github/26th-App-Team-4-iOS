...
//    super.viewDidAppear(animated)
//    goalValueTextField.becomeFirstResponder()
//    // Inform reactor that view appeared (e.g., for initial focus logic if needed)
//  }
//
//  override func viewWillDisappear(_ animated: Bool) {
//    super.viewWillDisappear(animated)
...
//    goalValueTextField.rx.text
//      .orEmpty
//      .skip(1) // Skip initial value set by reactor
//      .distinctUntilChanged()
...
//
//    // --- State Binding ---
//    // Update TextField text from Reactor's state
//    reactor.state.map { String($0.goalValue) }
//      .distinctUntilChanged()
//      .bind(to: goalValueTextField.rx.text)
//      .disposed(by: disposeBag)
//
//    // Handle loading state (disable user interaction)
//    reactor.state.map { $0.isLoading }
//      .distinctUntilChanged()
//      .map { !$0 } // Fix: Use map { !$0 } instead of .not()
//      .bind(to: view.rx.isUserInteractionEnabled)
//      .disposed(by: disposeBag)
//
//    // Handle save success to trigger animation
//    reactor.state.map { $0.saveSuccess }
//      .distinctUntilChanged { $0 == $1 } // Fix: Explicit comparison for Optional<Bool>
//      .subscribe(onNext: { [weak self] success in
//        guard let self, let success = success else { return }
//        if success {
//          // Animation is now triggered by saveSuccess
//          self.animationView.isHidden = false
//          self.animationView.play { [weak self] finished in // Add completion block
//            guard let self = self else { return }
//            if finished {
//              // Animation finished, now tell the Reactor to navigate
...
//
//
//    // Handle navigation side effects (one-time events)
//    reactor.state.compactMap { $0.navigationTarget }
//      .distinctUntilChanged() // Prevents re-triggering if same target emitted
...
//        case .showRunning: // No associated values now
...
//      .distinctUntilChanged { $0.localizedDescription == $1.localizedDescription } // Only react to new errors
//      .observe(on: MainScheduler.instance)
//      .subscribe(onNext: { error in
//        // Present an alert or toast message to the user
...
//
//
//    // Keyboard height handling (still local to VC as it's purely UI layout)
//    keyboardHeight
//      .distinctUntilChanged()
//      .subscribe(onNext: { [weak self] height in
//        guard let self = self else { return }
...
//  }
//
//  // MARK: - Underline Animation (still controlled by delegate, but could be moved to Reactor if desired)
//
//  private func animateUnderline(isVisible: Bool) {
//    if isVisible {
//      self.goalValueUnderline.transform = CGAffineTransform(scaleX: 0.01, y: 1.0)
//      self.goalValueUnderline.isHidden = false
//      UIView.animate(withDuration: 0.25) { // Add animation for appearance
//        self.goalValueUnderline.transform = .identity
//      }
//    } else {
//      UIView.animate(withDuration: 0.25, animations: {
...
//
//  func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
//    // This logic remains here for immediate input validation before sending to Reactor
//    if textField.text == "0" && string.isEmpty {
//      return false // Prevent deleting the initial '0' if it's the only character
//    }
//
//    let allowedCharacters = CharacterSet.decimalDigits
//    let characterSet = CharacterSet(charactersIn: string)
...
//    guard allowedCharacters.isSuperset(of: characterSet) else { return false }
//
//    // Handle leading zero for multi-digit numbers
//    if newText.count > 1 && newText.first == "0" && string != "" {
//      textField.text = String(newText.dropFirst())
//      // Manually trigger action for the corrected text so Reactor updates its state
//      reactor?.action.onNext(.updateGoalValue(textField.text ?? "0"))
//      return false // Prevent default text field update
//    }
//
//    let maxLength = 4
//    if newText.count > maxLength {
...
//    }
//
//    return true // Allow the change, Reactor's rx.text binding will pick up the new text
//  }
//}
